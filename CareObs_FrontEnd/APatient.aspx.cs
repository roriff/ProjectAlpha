...
public partial class APatient : System.Web.UI.Page
{
   ...
   public Int32 ViewAllPatients(string firstNameFilter, string lastNameFilter)
    ...
    protected void PatientDataGridView_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            Response.Redirect("APatientPage.aspx?PatientID=" + PatientID);
        }
        catch
        {
            //System Error;
    ...
    {
        //Navigate to the patient page to add a patient details on the form 
        // Redirect to a patient form page with PatientID = -1
        Response.Redirect("APatientPage.aspx?PatientID=" + -1); // this is a must as we are adding a patient so its PatientID=-1
    }
    ...
    protected void btnEditPatient_Click(object sender, EventArgs e)
    {
        ...
        lblPatientMessage.Text = "Search and select a patient from the list in order to edit patient's details."; // Message to user
    ...
    {
        //lblPatientMessage.Text = ""; // clear any previous message displayed first
        //lblPatientMessage.Text = "Search and select a patient from the list in order to delete patient."; // Message to user

        //Response.Redirect("ArchivePatient.aspx");

        PatientsInDataGridView("", "");// display the grid view first
        lblPatientMessage.Text = ""; // clear any previous message displayed first
        lblPatientMessage.Text = "Search and select a patient from the list in order to archive a patient."; // Message to user
        lblPatientMessage.ForeColor = System.Drawing.Color.Red;// set the message color to red to attract the attention of the user
    }
