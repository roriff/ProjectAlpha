...
public partial class AnObservation : System.Web.UI.Page
{
    ...
    {
        //Navigate to the observation page to add a patient observation details on the form 
        // Redirect to an observation form page with ObservationID = -1
        //Response.Redirect("AnObservationPage.aspx?ObservationID=" + -1); // this is a must as we are adding a new observation so its ObservationID=-1

        /*Change in requirements means we can only add an observation per patient, 
         * we can't view observations nor edit them as once there saved that'it
         *We might be able to disaply them in a kind of form for rport purposes in the back end
         *When we want to view the observation of a particular patient...*/
        PatientsInDataGridView("", "");// display the grid view first
        lblPatientMessage.Text = ""; // clear any previous message displayed first
        lblPatientMessage.Text = "Search and select a patient from the list in order to add patient's observation details."; // Message to user
        lblPatientMessage.ForeColor = System.Drawing.Color.Red;// set the message color to red to attract the attention of the user
    }

    protected void btnArchiveObservation_Click(object sender, EventArgs e)
    {
        Response.Redirect("ArchiveObservation.aspx");
    }
